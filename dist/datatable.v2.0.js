...
class DataTable {
  ...
  gmcol = () => {
    ...
    _.qa(_.s + " table", (n, i) => {
      ...
      for (var i = 0; i < n.rows.length; i++) {
        if (max < n.rows[i].cells.length) max = n.rows[i].cells.length;
      }
      _.gmcold = max;
    });
  };
      ...
      _.qa(".dt-body .dt-pagination-btn", (n, i) => {
        if (e.target == n || n.contains(e.target)) {
          if (n.hasAttribute("data")) {
            ...
            pn.querySelector(".dt-info-start").innerText =
              d.length == 0 ? 0 : (cp - 1) * _.clmt + 1;
            pn.querySelector(".dt-info-end").innerText =
              d.length < cp * _.clmt ? d.length : cp * _.clmt;
  ...
  dtlmt = (n) => {
    let _ = this,
      v = n.querySelector(".dt-limit").value,
  ...
  dtcs = (c, cid, t, n) => {
        ...
        if (cid === "dt-default-search") {
          _.qa(
            ...
            (tdv, zc) => {
              ...
              if (
                !so ||
                (Array.isArray(so) && so.length === 0) ||
                (typeof so === "object" && Object.keys(so).length === 0)
              ) {
                so = "column";
              }
              if (so) {
                ...
                if (t !== "") {
                  if (
                    (soa.includes("strict") && soa.includes("column")) ||
                    !soa.includes("strict")
                  ) {
          ...
          let ctd = cn.querySelector("td:nth-child(" + ci + ")" + cl),
            ctdt;
          if (!ctd) {
            ctdt = "";
          } else if (a) {
    ...
    if (tb.querySelectorAll(".dt-norow").length !== 0 || x === 0) {
      if (tb.querySelectorAll(".dt-norow").length == 0) {
        ...
        _.gmcol();
        et.innerHTML = '<td colspan="' + _.gmcold + '">No Result found.</td>';
  ...
  pgmk = (d, cp, n) => {
    ...
    if (pgn) {
      ...
      pgn.splice(-1);
      pgn.splice(0, 1);
      ph += `<button class="dt-pagination-btn" ${
        pgn_f > 0 ? 'data="' + pgn_f + '"' : "disabled"
      } >Prev</button>`;
      pgn.forEach((v) => {
        if (v == "s") {
      ...
      ph += `<button class="dt-pagination-btn" ${
        pgn_l - 1 < tpg ? 'data="' + pgn_l + '"' : "disabled"
      } >Next</button>`;
    } else {
    ...
    if (n && n.querySelector(".dt-pagination")) {
      ...
      n.querySelector(".dt-info-start").innerText =
        d == 0 ? 0 : (cp - 1) * _.clmt + 1;
      n.querySelector(".dt-info-end").innerText =
        d < cp * _.clmt ? d : cp * _.clmt;
    }
    return ph;
  };
      ...
      if (_.lmt.constructor == Array) {
        ...
        for (let i in _.lmt) {
          if (i == "active" && _.clmt === false) {
            ...
            df += `<option value="${_.lmt[i]}" ${
              _.lmt[i] == _.clmt ? "selcted" : ""
            }>${_.lmt[i]}</option>`;
          }
        }
      }
